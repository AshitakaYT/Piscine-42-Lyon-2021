...
int	ft_check_params(int ac, char **av)
{
	...
	while (tab[i])
		i++;
	printf(" i : %d\n", i);
	if (!ft_sqrt(i)) // potentiellement mettre un is_sqrt
		return (0);
	while (tab[j])
	{
		if (tab[j][0] < '0' || tab[j][0] > ft_sqrt(i) + '0' || ft_strlen(tab[j]) != 1) // remplacer les 4 par une racine carree
			return (0);
		j++;
	}
	return (1);
...
int	ft_solve(char **tab, int size)
{
	...
	if (i < size - 1)
		...
		return (1);
	printf("j = %d\n", j);
	while (tab[i][j] < size - 1 + '0')
	{
		tab[i][j]++;
		if (ft_line_left_right2(tab[i], size, j) && ft_line_right_left2(tab[i], size, j) && ft_column_up_down2(tab, j, size, i) && ft_column_down_up2(tab, j, size, i) && ft_check_duplicate_line(tab[i], tab[i][j], size) && ft_check_duplicate_column(tab, j, tab[i][j], size))
		{
...
int	main(int argc, char **argv)
{
	...
	while (views[i])
		i++;
	size = ft_sqrt(i); // potentiellement mettre ft_sqrt() du nombre d'arguments, si pas de solution return error
